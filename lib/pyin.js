...
export class PYINDetector {
    ...
    detectPitch(audioFrames) {
        ...
        audioFrames.forEach((frame) => {
            ...
            const candidates = PYINCore.extractMultipleCandidates(this.cmndfBuffer, this.sampleRate);
    ...
    findPitch(audioBuffer) {
        ...
        if (result.voiced && result.frequency > 0) {
            ...
            if (stateIndex >= 0) {
                // Re-extract candidates for confidence calculation using pre-allocated buffers
