...
export class PYINCore {
     ...
     */
    static fillObservationLogProbabilities(states, candidates, nCands, outputLogProbs, stepsPerSemitone, candLogProbs, candLogFreqs, optimalFreqs = null) {
        ...
        for (let s = 0; s < numStates; s++) {
            ...
            if (!state.voiced) {
                ...
                if (optimalFreqs) optimalFreqs[s] = 0;
            } else {
                let bestLog = -100;
                let bestCandFreq = state.frequency;
                ...
                for (let i = 0; i < nCands; i++) {
                    ...
                    if (w > bestLog) {
                        bestLog = w;
                        bestCandFreq = candidates[i].frequency;
                    }
                }
                outputLogProbs[s] = bestLog;
                if (optimalFreqs) optimalFreqs[s] = bestCandFreq;
            }
        }
    }
    ...
    static viterbi(logInitial, transitions, logObserv, numFrames, numStates) {
        ...
        const voicedIdx = transitions.voicedIdx;
        const unvoicedIdx = transitions.unvoicedIdx[0];
        const voicedRanges = transitions.voicedRanges;
        ...
        for (let t = 1; t < numFrames; t++) {
            ...
            // 1. Voiced states
            for (let i = 0; i < voicedIdx.length; i++) {
                const s = voicedIdx[i];
                const range = voicedRanges[i];
                let maxV = -Infinity, bestP = 0;

                // Voiced -> Voiced
                for (let prev = range.start; prev <= range.end; prev++) {
                    const prob = prevP[prev] + logMatrix[prev * numStates + s];
                    if (prob > maxV) { maxV = prob; bestP = prev; }
                }

                // Unvoiced -> Voiced
                const uvProb = prevP[unvoicedIdx] + logMatrix[unvoicedIdx * numStates + s];
                if (uvProb > maxV) { maxV = uvProb; bestP = unvoicedIdx; }

                viterbiLogProb[currOff + s] = maxV + currO[s];
                backpointer[currOff + s] = bestP;
            }
     ...
     * 
     * WHY Analysis: O(N*K) implementation using pre-calculated transition ranges.
     * Voiced-to-voiced transitions are localized in frequency, allowing us to skip
     * calculations for distant pitch states.
     * 
     * @param {Float32Array} prevLogProbs - Probabilities from previous frame
     * @param {Object} transitions - Object containing the log-transition matrix and range data
    ...
    static forwardViterbiStep(prevLogProbs, transitions, currentLogObs, newLogProbs = null, backpointer = null) {
        ...
        const logMatrix = transitions.logMatrix;
        const voicedIdx = transitions.voicedIdx;
        const unvoicedIdx = transitions.unvoicedIdx[0]; // Assume single unvoiced state for now
        const voicedRanges = transitions.voicedRanges;
        ...
        for (let i = 0; i < voicedIdx.length; i++) {
            ...
            // Unvoiced -> Voiced
            const uvProb = prevLogProbs[unvoicedIdx] + logMatrix[unvoicedIdx * numStates + s];
            if (uvProb > maxV) { maxV = uvProb; bestP = unvoicedIdx; }
...
export class PYINTransitions {
    constructor(states, switchProb = 0.01) {
        ...
        for (let i = 0; i < numStates; i++) {
            if (states[i].voiced) this.voicedIdx.push(i); else this.unvoicedIdx.push(i);
        }
        ...
        for (let i = 0; i < numStates; i++) {
            ...
            for (let j = 0; j < numStates; j++) {
                ...
                if (sI.voiced === sJ.voiced) {
                    if (sI.voiced) {
                        ...
                        rowProbs[j] = 1.0 / Math.max(1, this.unvoicedIdx.length);
                    }
            ...
            const targetGroup = sI.voiced ? this.unvoicedIdx : this.voicedIdx;
        ...
        // Pre-calculate voiced transition ranges for optimization
        this.voicedRanges = this.voicedIdx.map(i => {
            let start = -1, end = -1;
            for (let j = 0; j < this.voicedIdx.length; j++) {
                const targetIdx = this.voicedIdx[j];
                // Use a threshold for transition probability to define the range
                // log(1e-6) approx -13.8. Gaussian tail at 5*sigma is approx -12.5.
                if (this.logMatrix[i * numStates + targetIdx] > -15) {
                    if (start === -1) start = targetIdx;
                    end = targetIdx;
                }
            }
            return { start, end };
...
export class PYINDetector {
    ...
    constructor(sampleRate, frameSize = 2048, minFreq = 80, maxFreq = 800, stepsPerSemitone = 5) {
        ...
        this.optimalFreqBuf = new Float32Array(this.states.length); // Buffer for optimal frequencies per state
    ...
    findPitch(buf) {
        ...
        PYINCore.fillObservationLogProbabilities(
            this.states, this.context.candidates, nC, this.obsBuf,
            this.stepsPerSemitone, this.context.candLogProbs, this.context.candLogFreqs,
            this.optimalFreqBuf
        );
        ...
        // Use the continuous optimal frequency if available, otherwise fallback to grid
        const continuousFreq = this.optimalFreqBuf[best];
        this.result[0] = (continuousFreq > 0) ? continuousFreq : state.frequency;
    ...
    detectPitch(frames) {
        ...
        const obsFreqs = new Float32Array(nF * nS); // Buffer for optimal frequencies across all frames
        ...
        for (let t = 0; t < nF; t++) {
            ...
            const nC = PYINCore.extractMultipleCandidates(this.cmndfBuf, this.sampleRate, this.context);
            PYINCore.fillObservationLogProbabilities(
                this.states, this.context.candidates, nC,
                obsL.subarray(t * nS, (t + 1) * nS),
                this.stepsPerSemitone,
                this.context.candLogProbs,
                this.context.candLogFreqs,
                obsFreqs.subarray(t * nS, (t + 1) * nS)
            );
        }
        ...
        return Array.from(path).map((stateIdx, t) => {
            const state = this.states[stateIdx];
            // Use the stored optimal frequency for this frame and state
            const accurateFreq = obsFreqs[t * nS + stateIdx];
            return {
                frequency: (accurateFreq > 0) ? accurateFreq : state.frequency,
                voiced: state.voiced
            };
        });
    }
