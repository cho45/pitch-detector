...
export class PYINDetector {
    ...
    detectPitch(audioFrames) {
        ...
        audioFrames.forEach((frame) => {
            PYINCore.calculateDifferenceFunction(frame, this.differenceBuffer);
            ...
            const probabilities = PYINCore.calculatePitchProbabilities(candidates, this.cmndfBuffer, this.sampleRate);
    ...
    findPitch(audioBuffer) {
        // Lightweight, single-frame version without HMM
        ...
        // Determine voiced/unvoiced based on the best candidate's properties
        // A simple heuristic: if the best candidate has a very low probability or high CMNDF value, consider it unvoiced.
        const cmndfVal = bestCandidate.tau > 0 && bestCandidate.tau < this.cmndfBuffer.length ? this.cmndfBuffer[Math.round(bestCandidate.tau)] : 1.0;
        const confidence = bestCandidate.probability * (1 - cmndfVal);

        if (confidence > 0.1) { // Voiced threshold
