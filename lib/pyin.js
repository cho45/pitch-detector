...
export class PYINCore {
     ...
     */
    static fillObservationLogProbabilities(states, candidates, nCands, outputLogProbs, stepsPerSemitone = 5) {
        const numStates = states.length, sigmaScale = -1 / (2 * 50 * 50), centsScale = 1200;
        let totalVoicedProb = 0;
        for (let i = 0; i < nCands; i++) totalVoicedProb += candidates[i].probability;
        ...
        const candLogProbs = new Float32Array(nCands), candLogFreqs = new Float32Array(nCands);
...
export class PYINDetector {
    ...
    findPitch(buf) {
        ...
        PYINCore.fillObservationLogProbabilities(this.states, this.context.candidates, nC, this.obsBuf, this.stepsPerSemitone);
        ...
        return [state.frequency, sumV / sumT];
    }
    ...
    detectPitch(frames) {
        ...
        for (let t = 0; t < nF; t++) {
            ...
            PYINCore.calculateCMNDF(this.diffBuf, this.cmndfBuf);
            const nC = PYINCore.extractMultipleCandidates(this.cmndfBuf, this.sampleRate, this.context);
            PYINCore.fillObservationLogProbabilities(this.states, this.context.candidates, nC, obsL.subarray(t * nS, (t + 1) * nS), this.stepsPerSemitone);
        }
