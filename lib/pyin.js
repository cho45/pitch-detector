...
export class PYINCore {
    ...
    static calculateObservationProbabilities(states, observations) {
        ...
        const voicedStatesCount = states.filter(s => s.voiced).length;
        ...
        for (let t = 0; t < numObs; t++) {
            ...
            // WHY Analysis: State-Density Normalization
            // In the HMM, we have ~hundreds of voiced states but usually only one unvoiced state.
            // If we assign the raw unvoiced probability (1 - totalVoicedProb) to the single unvoiced state,
            // it will have a much higher likelihood than any individual voiced state, even if totalVoicedProb is 0.4.
            // To fix this "unvoiced dominance", we divide the unvoiced probability by the number of voiced states,
            // effectively comparing the likelihood of "this specific pitch" vs "silence/noise spread across the range".
            const rawUnvoicedProb = Math.max(1e-15, 1 - Math.min(1, totalVoicedProb));
            const densityNormalizedUnvoicedLogProb = Math.log(rawUnvoicedProb / voicedStatesCount);

            for (let s = 0; s < numStates; s++) {
                const state = states[s];
                if (!state.voiced) {
                    result[t][s] = densityNormalizedUnvoicedLogProb;
                } else {
...
class PYINTransitions {
    constructor(states, switchProb = 0.002) {
        const numStates = states.length;
...
export class PYINDetector {
    ...
    findPitch(buf) {
        ...
        // 2. Observation log-probabilities (Use centralized logic)
        const obsProbs = PYINCore.calculateObservationProbabilities(this.states, [cands])[0];
        for (let s = 0; s < numStates; s++) {
            this.obsBuf[s] = Math.log(Math.max(1e-15, obsProbs[s]));
        }
