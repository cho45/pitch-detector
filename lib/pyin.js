...
export class PYINCore {
    ...
    /**
     * Perform a single forward step of the Viterbi algorithm.
     * Calculates new log probabilities for each state based on previous probabilities,
     * transition matrix, and current observation probabilities.
     * 
     * @param {Float32Array} prevLogProbs - Log probabilities from previous step [numStates]
     * @param {Array<Float32Array>} logTransitions - Log transition matrix [numStates][numStates]
     * @param {Float32Array} currentLogObs - Log observation probabilities for current frame [numStates] (or flat array with offset)
     * @param {number} obsOffset - Offset in currentLogObs if it's a flat array
     * @param {Int32Array} backpointer - Optional output array to store best previous state indices [numStates]
     * @returns {Float32Array} New log probabilities [numStates]
     */
    static forwardViterbiStep(prevLogProbs, logTransitions, currentLogObs, obsOffset = 0, backpointer = null) {
        const numStates = prevLogProbs.length;
        const newLogProbs = new Float32Array(numStates);
        ...
        for (let s = 0; s < numStates; s++) {
            ...
            for (let prev = 0; prev < numStates; prev++) {
                const transLogProb = logTransitions[prev][s];
                if (transLogProb === -Infinity) continue;

                const prob = prevLogProbs[prev] + transLogProb;
            ...
            newLogProbs[s] = maxLogProb + currentLogObs[obsOffset + s];
            if (backpointer) {
                backpointer[s] = bestPrevState;
            }
        }
     ...
     * @param {Float32Array} logInitial - Log probabilities of initial states [numStates]
     * @param {Array<Float32Array>} logTransitions - Log transition matrix [numStates][numStates]
     * @param {Float32Array} logObserv - Log observation probabilities [numFrames * numStates] flat array
    ...
    static viterbi(logInitial, logTransitions, logObserv, numFrames, numStates) {
        // Viterbi variables: flat arrays for performance
        ...
        // Helper buffer for step backpointer
        ...
        for (let t = 1; t < numFrames; t++) {
            ...
            const nextProbs = PYINCore.forwardViterbiStep(prevProbs, logTransitions, logObserv, currentOffset, stepBackpointer);

            // Copy results back to main matrix
            for (let s = 0; s < numStates; s++) {
        ...
        // Termination & Traceback (Backward pass)
        let lastBestState = 0;
        let maxFinalLogProb = -Infinity;
        const finalOffset = (numFrames - 1) * numStates;

        for (let s = 0; s < numStates; s++) {
            if (viterbiLogProb[finalOffset + s] > maxFinalLogProb) {
                maxFinalLogProb = viterbiLogProb[finalOffset + s];
                lastBestState = s;
            }
        }
        ...
        let currentState = lastBestState;

        path[numFrames - 1] = currentState;
        for (let t = numFrames - 1; t > 0; t--) {
            currentState = backpointer[t * numStates + currentState];
            path[t - 1] = currentState;
        }
...
export class PYINDetector {
    ...
    /**
     * Detect pitch in multiple audio frames by processing them as a batch using offline Viterbi.
     * This provides the global optimal path using Forward-Backward algorithm principles.
    ...
    detectPitch(audioFrames) {
        const numFrames = audioFrames.length;
        const numStates = this.states.length;

        // Pre-calculate observation probabilities for all frames
        const obsLogProbs = new Float32Array(numFrames * numStates);

        for (let t = 0; t < numFrames; t++) {
            // Step 1: Extract candidates
            PYINCore.calculateDifferenceFunction(audioFrames[t], this.differenceBuffer);
            ...
            // Step 2: Calculate observation probabilities
            const obsProb = PYINCore.calculateObservationProbabilities(this.states, [candidates]);

            for (let s = 0; s < numStates; s++) {
                const p = obsProb[0][s];
                const logP = p > 1e-15 ? Math.log(p) : -34.5; // log(1e-15) ≈ -34.5
                obsLogProbs[t * numStates + s] = logP;
            }
        }

        // Initial probabilities
        const voicedStatesCount = this.states.filter(s => s.voiced).length;
        const initLogVoiced = voicedStatesCount > 0 ? Math.log(0.9 / voicedStatesCount) : -Infinity;
        const initLogUnvoiced = Math.log(0.1);
        const logInitial = new Float32Array(numStates);

        for (let s = 0; s < numStates; s++) {
            logInitial[s] = this.states[s].voiced ? initLogVoiced : initLogUnvoiced;
        }

        // Run Viterbi
        const path = PYINCore.viterbi(logInitial, this.logTransitions, obsLogProbs, numFrames, numStates);

        // Build the result
        const pitchTrack = new Array(numFrames);
        for (let t = 0; t < numFrames; t++) {
            const state = this.states[path[t]];
            pitchTrack[t] = {
    ...
    /**
     * Process a single audio frame greedily for online processing.
     * Note: For best results on recorded audio, use detectPitch() for offline global optimization.
    ...
    findPitch(audioBuffer) {
        ...
        // Step 3: Update Viterbi path (Forward step in online Viterbi)
        if (!this.isInitialized) {
            ...
            for (let i = 0; i < numStates; i++) {
                const initialProb = this.states[i].voiced ? initLogVoiced : initLogUnvoiced;
                this.viterbiLogProb[i] = initialProb + obsLogProb[i];
            }
            this.isInitialized = true;
        } else {
            // Iteration step for subsequent frames using shared Forward logic
            const prevViterbiLogProb = this.viterbiLogProb;
            const newViterbiLogProb = PYINCore.forwardViterbiStep(prevViterbiLogProb, this.logTransitions, obsLogProb);
        ...
        // Log-sum-exp is safer here but direct exp is usually okay relative to max
        let probSum = 0;
        const maxP = this.viterbiLogProb[bestStateIndex];
        // Limit calculation to values close to max to avoid underflow/precision issues
        for (let i = 0; i < numStates; i++) {
            const diff = this.viterbiLogProb[i] - maxP;
            if (diff > -20) { // only consider probability masses within e^-20 factor
                probSum += Math.exp(diff);
            }
        }
