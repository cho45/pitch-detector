...
export class PYINCore {
     ...
     */
    static fillObservationLogProbabilities(states, observations, outputLogProbs, stepsPerSemitone = 5) {
        ...
        // WHY Analysis: Resolution-Independent Density Normalization
        // Instead of dividing by the discrete number of states (vCount), which introduces 
        // a bias when resolution changes, we normalize against the "width" of a single bin.
        // In our MIDI-aligned grid, each bin covers (1 / stepsPerSemitone) semitones.
        const rawUnvoicedProb = Math.max(1e-15, 1 - Math.min(1, totalVoicedProb));
        const unvoicedLogProb = Math.log(rawUnvoicedProb / stepsPerSemitone);

        // Pre-calculate candidate log weights and frequencies to avoid O(N*M) log calls
        const nObs = observations.length;
        const candLogProbs = new Float32Array(nObs);
        const candLogFreqs = new Float32Array(nObs);
        for (let i = 0; i < nObs; i++) {
            const c = observations[i];
            candLogProbs[i] = Math.log(Math.max(1e-15, c.probability));
            candLogFreqs[i] = Math.log2(c.frequency);
        }
        ...
        for (let s = 0; s < numStates; s++) {
            ...
            if (!state.voiced) {
                ...
                const sLog2 = state.log2freq;
                for (let i = 0; i < nObs; i++) {
                    const diffCents = centsScale * (candLogFreqs[i] - sLog2);
                    const logWeight = candLogProbs[i] + (diffCents * diffCents * sigmaScale);
...
class PYINTransitions {
    ...
    constructor(states, switchProb = 0.01) {
        ...
        const sigmaTrans = 25; // cents (from paper Mauch & Dixon 2014 Section 3.2.2)
...
export class PYINDetector {
     ...
     * @param {number} frame_size - FFT window size
    ...
    constructor(sampleRate, frameSize = 2048, minFreq = 80, maxFreq = 800, stepsPerSemitone = 5) {
        ...
        this.stepsPerSemitone = stepsPerSemitone;
        ...
        this.nextViterbiBuf = new Float32Array(this.states.length); // Avoid new in findPitch
    ...
    findPitch(buf) {
        ...
        PYINCore.fillObservationLogProbabilities(this.states, cands, this.obsBuf, this.stepsPerSemitone);
        ...
        if (!this.isInitialized) {
            ...
            PYINCore.forwardViterbiStep(this.viterbiLogProb, this.transitions, this.obsBuf, this.nextViterbiBuf);
            // Swap buffers to avoid allocation
            const tmp = this.viterbiLogProb;
            this.viterbiLogProb = this.nextViterbiBuf;
            this.nextViterbiBuf = tmp;
        }
    ...
    detectPitch(frames) {
        ...
        for (let t = 0; t < nFrames; t++) {
            ...
            PYINCore.fillObservationLogProbabilities(this.states, allCands[t], frameLogs, this.stepsPerSemitone);
        }
