...
export function betaCdf(x, a, b) {
    ...
    // For beta(2, 18), use closed form: CDF(x) = 1 - (1-x)^18 * (1 + 18*x)
    if (a === 2 && b === 18) {
        const oneMinusX = 1 - x;
        const oneMinusXPow18 = Math.pow(oneMinusX, 18);
        return 1 - oneMinusXPow18 * (1 + 18 * x);
    }
...
export function boltzmannPmf(k, a, n) {
    ...
    // Use geometric series formula for O(1) normalization constant:
    // sum_{i=0}^{n-1} e^(-a*i) = (1 - e^(-a*n)) / (1 - e^(-a))
    const r = Math.exp(-a);
    if (Math.abs(1 - r) < 1e-10) return 1 / n; // Handle a approx 0

    const sum = (1 - Math.pow(r, n)) / (1 - r);
    return Math.pow(r, k) / sum;
}
...
/**
 * Calculate log of sum of exponentials in a numerically stable way
 * log(sum(exp(x_i))) = max(x) + log(sum(exp(x_i - max(x))))
 * @param {...number} args - Log probabilities
 * @returns {number} Log of sum of exponentials
 */
export function logSumExp(...args) {
