...
import { betaCdf, boltzmannPmf, generateThresholds, calculateBetaProbabilities, logSumExp } from '../lib/statistics.js';
...
describe('logSumExp', () => {
    test('basic calculation', () => {
        const a = 1;
        const b = 2;
        const expected = Math.log(Math.exp(a) + Math.exp(b));
        const actual = logSumExp(a, b);
        assertApproxEqual(actual, expected, 1e-10);
    });

    test('underflow protection (very small values)', () => {
        // log(e^-1000 + e^-1000) = log(2 * e^-1000) = log(2) - 1000
        const a = -1000;
        const b = -1000;
        const actual = logSumExp(a, b);
    ...
    test('handling infinity', () => {
        assertApproxEqual(logSumExp(-Infinity, 1), 1, 1e-10);
        assertApproxEqual(logSumExp(1, -Infinity), 1, 1e-10);
        assert.strictEqual(logSumExp(-Infinity, -Infinity), -Infinity);
    });
});
